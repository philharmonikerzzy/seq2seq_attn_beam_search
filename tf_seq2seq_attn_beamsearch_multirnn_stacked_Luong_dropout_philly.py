...
import os, math,logging, argparse, sys
...
with graph.as_default():
	...
	################################################### ends initialization 
	#set up encoder 
    forward_cell = tf.contrib.rnn.MultiRNNCell([DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(num_nodes),output_keep_prob = 0.7) for _ in range(num_layers)])
    backward_cell = tf.contrib.rnn.MultiRNNCell([DropoutWrapper(tf.nn.rnn_cell.BasicLSTMCell(num_nodes), output_keep_prob = 0.7) for _ in range(num_layers)])
    ...
    encoder_outputs = tf.concat([encoder_output_fw, encoder_output_bw], axis = 2)
    encoder_last_state = []
    ...
    with tf.variable_scope('shared_attention_mechanism'):
        attention_mechanism = seq2seq.LuongAttention(num_units = hidden_dim*2, memory = encoder_outputs, memory_sequence_length = encoder_inputs_length)
...
def add_arguments(parser):
    ...
    # additional misc arguments
    parser.add_argument("--windows", type="bool", default="False", help="whether the script is being run on windows or linux/unix")
    parser.add_argument("--input-training-data-path", type=str, default="", help="path of input data.")
    parser.add_argument("--input-validation-data-path", type=str, default="", help="path of input data.")
    parser.add_argument("--input_previous_model_path", type=str, default="", help="path of previous model.")
    parser.add_argument("--output-model-path", type=str, default="", help="path for output model.")
    parser.add_argument("--logdir", type=str, default="",  help="Directory for log files")
    parser.add_argument("--previous_model_name", type=str, default="", help="filename of previous model.")
    parser.add_argument("--training_file_name", type = str, default = "Train.txt", help = "filename of the training file")
    parser.add_argument("--test_file_name", type = str, default = "Test.txt", help = "filename of the test file")
    ...
    parser.add_argument("--epochs", type=int, default=50, help="epochs over whole data")
    parser.add_argument("--batch_size", type=int, default=100, help="examples per batch")
    parser.add_argument("--steps_per_epoch", type=int, default=10000, help="steps in each epochs")
    parser.add_argument("--validation_steps", type=int, default=1000, help="steps in validation")
    parser.add_argument("--early_stop_min_delta", type=float, default=0.001, help="min_delta in early stop")
    parser.add_argument("--early_stop_patience", type=int, default=10, help="patience in early stop")
    parser.add_argument("--enable_randomization", type="bool", default="False", help="enable randomization")
    parser.add_argument("--save_weights_only", type="bool", default="True", help="enable randomization")
    parser.add_argument("--save_best_only", type="bool", default="True", help="enable randomization")
...
if __name__ == "__main__":
	...
	parsed_args, unparsed_args = argument_parser.parse_known_args()
	logging.info("Parsed Args:" + str(parsed_args))
	logging.info("Unparsed Args: " + str(unparsed_args))
	num_steps = 100000
	print("data is loaded")
	tf.gfile.MakeDirs(parsed_args.output_model_path + '/tf_s2s_bs_models_decoder_shared/')
	with tf.Session(graph=graph) as session:
		tf.global_variables_initializer().run()   #initializes all variables at once"""
		mean_loss = 0
		print("ready to start training")
		step = 0
		test_generator = reader.data_generator_s2s_train_philly(parsed_args.input_training_data_path, '/Test_March_2016_1C.tsv', tag_to_id, 15, 1)
		for batch_features, batch_labels, lengths, labels in reader.data_generator_s2s_train_philly(parsed_args.input_training_data_path, '/CombinedTraining_removed_test.tsv',tag_to_id,15,32, True):
			if step > num_steps:
			    break
			...
			if step%5000 == 0:
			 	...
			 	saver.save(session, save_path = parsed_args.output_model_path + '/tf_s2s_bs_models_decoder_shared/s2s_per500', global_step = global_step)
		print("Saving trained model")
		#saver.save(session, save_path = './tf_seq2seq_beam_search_random_decoder_shared/', global_step = global_step)		
