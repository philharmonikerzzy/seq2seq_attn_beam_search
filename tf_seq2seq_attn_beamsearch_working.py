...
with graph.as_default():
	...
	################################################### ends initialization 
	#set up encoder 
    forward_cell = tf.nn.rnn_cell.BasicLSTMCell(num_nodes)
    backward_cell = tf.nn.rnn_cell.BasicLSTMCell(num_nodes)
    #input_layer = Dense(num_nodes, dtype=tf.float32, name='input_projection')
    #encoder_inputs = input_layer(encoder_inputs)
    ...
    encoder_last_state_c = tf.concat([encoder_last_state_fw[0], encoder_last_state_bw[0]],1)
    encoder_last_state_h = tf.concat([encoder_last_state_fw[1],encoder_last_state_bw[1]],1)
    encoder_last_state = tf.contrib.rnn.LSTMStateTuple(c = encoder_last_state_c, h = encoder_last_state_h)
    ...
    decoder_cell = seq2seq.AttentionWrapper(cell = tf.nn.rnn_cell.BasicLSTMCell(hidden_dim*2),
    ...
    training_helper = seq2seq.TrainingHelper(
        ...
        sequence_length = tf.tile(tf.constant([20], dtype = tf.int32), [batch_size]),#decoder_inputs_length_train,
        time_major = False
    )
    ...
    with tf.variable_scope('decode_with_shared_attention'):
        training_decoder_output, _, _ = seq2seq.dynamic_decode(
            ...
            maximum_iterations = 20 #tf.reduce_max(decoder_inputs_length_train)
        )
    ...
    masks = tf.sequence_mask(lengths = decoder_inputs_length_train, maxlen= 20, dtype = tf.float32)
...
if __name__ == "__main__":
	...
	with tf.Session(graph=graph) as session:
		...
		step = 0
		test_generator = reader.data_generator_s2s_train('Test_March_2016_1C.tsv', tag_to_id, 20, 1)
		for batch_features, batch_labels, lengths, labels in reader.data_generator_s2s_train('CombinedTraining_removed_test.tsv',tag_to_id,20,32):
			if step > num_steps:
			    break
			...
			if step%500 == 0:
			 	...
			 	saver.save(session, save_path = './tf_s2s_bs_models/s2s_per500', global_step = global_step)
		print("Saving trained model")
		saver.save(session, save_path = './tf_seq2seq_beam_search/', global_step = global_step)		
